...
export default function DashboardPage() {
  ...
  if (!token) {
    return (
      <main className="page-container max-w-xl py-12">
        <div className="glass-card p-8 text-center rounded-2xl border border-white/10">
          <p className="text-muted mb-6">Log in to view your dashboard.</p>
  ...
  return (
    <main className="page-container">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted mt-0.5">Your campaigns and activity — watch, share and earn.</p>
      </div>
      {/* One brand creative per campaign — users watch, share, like */}
      <section className="glass-card p-4 mb-6 rounded-2xl border border-white/10 bg-white/5">
        <p className="text-sm text-foreground/90">
          <strong>One viral creative per campaign.</strong> The brand uploads one creative; you watch, share and like it to support the campaign and grow the brand.
        </p>
      </section>
        ...
        <section className="glass-card p-6 sm:p-8 mb-8 overflow-hidden relative rounded-2xl border border-white/10">
          ...
          <div className="relative">
            ...
            <Link href={`/campaign/${hero.id}`} className="inline-block mt-5">
              <Button className="rounded-xl">View campaign & share creative</Button>
            </Link>
          </div>
        </section>
      ) : (
        <section className="glass-card p-8 mb-8 rounded-2xl border border-white/10 space-y-4">
          <p className="text-foreground font-medium">No active campaigns right now.</p>
          <p className="text-sm text-muted">
            Each campaign has <strong>one brand creative</strong> (uploaded by admin). Users watch, share and like it — one creative goes viral per campaign.
          </p>
          <p className="text-sm text-muted">
            Admins: create a campaign in <Link href="/admin/campaigns" className="text-primary hover:underline">Admin → Campaigns</Link>, then open it → <strong>Brand creative</strong> tab → upload the image or video. Set status to Active so users see it.
          </p>
        </section>
      )}
